                ...
                function01-hp2530 -command "dhcp config-file-update"
                    ...
                    Write-Host $streamoutput
                        ...
                        Add-Content -Path $pathlog -Value $log
                            ...
                            {
                            Write-Host "`n`n#####$date - Validacao e escrita de Firmware no filesystem do Switch de IP $ipaddress DEU CERTO!!! Iniciando configuração de JOB para reboot de madrugada#####`n`n"
                            [string]$log = $date.ToString() +" - Validacao e escrita de Firmware no filesystem do Switch de IP "+ $ipaddress.ToString() + " DEU CERTO!!! Iniciando configuração de JOB para reboot de madrugada."
                            Add-Content -Path $pathlog -Value $log
